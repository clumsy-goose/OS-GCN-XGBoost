...
data<-read.csv(file = "../.././dataProcessedResult/phenotype_rpkm/DTF.csv",header = T,row.names='Entry_Treatment_Replicate') 
...
for (i in 1:n){
  for (j in 0:jj) {
    ...
    train.trait <- training$DTF 
    test.trait <- testing$DTF
    new_training <- model.matrix(~.+0,data = training[,-c("DTF"),with=F]) 
    new_testing <- model.matrix(~.+0,data = testing[,-c("DTF"),with=F])
...
ggplot(plot_data, aes(x = reorder(Gene, SUM), y = SUM)) +
  ...
  labs(
    title = "Top Genes by Composite Importance Score (Days till flowering (50%) (DTF))",
    x = "Gene",
    y = "Importance Score"
  ) +
  coord_flip() +  # 翻转坐标轴
