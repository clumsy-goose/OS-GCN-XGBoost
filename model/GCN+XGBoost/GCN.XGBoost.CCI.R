...
data<-read.csv(file = "../.././dataProcessedResult/phenotype_rpkm/CCI.csv",header = T,row.names='Entry_Treatment_Replicate') 
...
for (i in 1:n){
  for (j in 0:jj) {
    ...
    train.trait <- training$CCI 
    test.trait <- testing$CCI
    new_training <- model.matrix(~.+0,data = training[,-c("CCI"),with=F]) 
    new_testing <- model.matrix(~.+0,data = testing[,-c("CCI"),with=F])
...
ggplot(plot_data, aes(x = reorder(Gene, SUM), y = SUM)) +
  ...
  labs(
    title = "Top Genes by Composite Importance Score (Chlorophyll content index (CCI))",
    x = "Gene",
    y = "Importance Score"
  ) +
  coord_flip() +  # 翻转坐标轴
