...
# 1. Load data -----------------------------------------------------------------------------------
data<-read.csv(file = "../.././dataProcessedResult/phenotype_rpkm/Fecundity.csv",header = T,row.names='Entry_Treatment_Replicate') 
gene_scores <- read.csv("../.././dataProcessedResult/geneWeight/GCN_scores.csv", header = TRUE, stringsAsFactors = FALSE)
...
gene_scores$gene_id <- as.character(gene_scores$gene_id)

for (gene in colnames(data)) {
  if (gene %in% gene_scores$gene_id) {
    score_value <- gene_scores$score[gene_scores$gene_id == gene]
    data[[gene]] <- data[[gene]] + score_value
  }
}
# 2. XGBoost -------------------------------------------------------------------------------------
...
for (i in 1:n){
  for (j in 0:jj) {
    ...
    train.trait <- training$Fecundity 
    test.trait <- testing$Fecundity
    new_training <- model.matrix(~.+0,data = training[,-c("Fecundity"),with=F]) 
    new_testing <- model.matrix(~.+0,data = testing[,-c("Fecundity"),with=F])
...
save(data,obs, pred, impt.out, rmse, train.rmse,k,n,jj,r,c,colsample,eta,num_parallel_tree,subsample,
     file="./Fecundity.RData")

## Organize cor

load(file = "./Fecundity.RData")
...
ggplot(plot_data, aes(x = reorder(Gene, SUM), y = SUM)) +
  ...
  labs(
    title = "Top Genes by Composite Importance Score (Fecundity (Filled grain number))",
    x = "Gene",
    y = "Importance Score"
  ) +
  coord_flip() +  # 翻转坐标轴
